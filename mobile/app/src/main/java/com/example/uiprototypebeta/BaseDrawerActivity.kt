...
open class BaseDrawerActivity : AppCompatActivity() {
    ...
    private fun onAdminClicked() {
        ...
        if (AdminSession.isLoggedIn) {
            val currentAdminPath = intent.getStringExtra(AdminDashboardActivity.extraPath).orEmpty()
            val isAdminOverview = this is AdminDashboardActivity &&
                (currentAdminPath.isBlank() || currentAdminPath == "/admin/dashboard")
            if (!isAdminOverview) {
